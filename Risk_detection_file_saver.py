...
# ---------------------------------------------------------
# Identifies students who are academically at risk
# based on:
#   - Average mark < 50
#   - OR failing 2 or more courses
#
# Uses:
#   - marks table
#   - students table
...
def get_at_risk_students():
    """
    Returns a list of at-risk students with:
    - student_id
    - name
    - surname
    - average mark
    ...
    with get_connection() as conn:
        ...
        # Fetch student marks aggregated per student
        cur.execute("""
            SELECT 
                s.student_id,
                s.name,
                s.surname,
                AVG(m.final_mark) as average_mark,
                SUM(CASE WHEN m.final_mark < 50 THEN 1 ELSE 0 END) as failures
            FROM students s
        ...
        for row in results:
            avg = row["average_mark"]
            failures = row["failures"]

            # Apply risk conditions
            if avg < 50 or failures >= 2:
...
# ---------------------------------------------------------
# Q9: FILE HANDLING (EXPORT DATA)
# ---------------------------------------------------------
# Exports all system data into a JSON file
# ---------------------------------------------------------
import json
...
def export_data(filename="backup.json"):
    ...
    Acts as a backup of the system.
    """
    with get_connection() as conn:
        cur = conn.cursor()
...
# ---------------------------------------------------------
# Q9: FILE HANDLING (IMPORT DATA)
# ---------------------------------------------------------
# Loads data from JSON back into the database
# ---------------------------------------------------------
